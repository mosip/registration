## MOSIP Registration Database scripts inventory and deployment guidelines on postgresql database. 
...
**Example:** the registration module script folder is /**mosip_base_directory**>>db_scripts>>mosip_regprc where all the database scripts related to registration processor are available.
...
* **mosip_registration_db_deployment.sh:** This is the .sh file which is present in /home/madmin/database directory and which will be executed for all database deployment in single command execution.
...
1) DB Deployment for all Registration databases
...
## DB Deployment for all Registration databases with single click deployment:
...
**Step 2** -> Deployment on all databases, run the **"mosip_registration_db_deployment.sh"** script which is avialble in the /database directory.  To access **"mosip_registration_db_deployment.sh"** script, follow the below given commands:

<em> **Enter:-bash-4.2$** cd /home/madmin/database/ </em>

<em> **Enter:-bash-4.2$** bash mosip_registration_db_deployment.sh </em>
