...
* `A02 Create Machines` creates a large number of Machines based on requirements within the Master Database that contains unique keys. These unique keys can be generated by running [KeyGenMadeEasy](KeyGenMadeEasy/) script. To run the scitpt please follow this document [keygen.txt](KeyGenMadeEasy/keygen.txt)
...
# Script execution steps:
	...
	* A01 Create ZoneUsers and Centers (Setup) - This thread group will create a list of users and centers within the master DB and activate them. The centers will be based on the list of zones and location provided in "D01_A01_zones.csv". This step only needs to be executed once per environment.
...
2. [D01_A01_zones.csv](support-files/D01_A01_zones.csv) - This support file contains list of valid zones in master database where a cneter can be created.
...
5. [D02_S02T03_KeyAlias.csv](support-files/D02_S02T03_KeyAlias.csv) - This support file contains list of Certificate details from database.
