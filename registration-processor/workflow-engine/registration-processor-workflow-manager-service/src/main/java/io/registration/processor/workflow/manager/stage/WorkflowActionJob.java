...
import io.registration.processor.workflow.manager.constants.WorkflowManagerConstants;
...
public class WorkflowActionJob extends MosipVerticleAPIManager {
	...
	/** The fetch size. */
	@Value("${registration.processor.workflow.action.job.fetchsize}")
	private Integer fetchSize;

	/** The elapse time. */
	@Value("${registration.processor.workflow.action.job.elapse.time}")
	...
	public void schedulerResult(AsyncResult<String> res) {
		if (res.succeeded()) {
			...
			regProcLogger.error(LoggerFileConstant.SESSIONID.toString(), LoggerFileConstant.REGISTRATIONID.toString(),
					"", "WorkflowActionJob::schedular()::deploymemnt failure " + res.cause());
		}
	}
