...
public class QualityClassifierStage extends MosipVerticleAPIManager {
	...
	private Map<String, String> getQualityTags(List<BIR> birs) throws BiometricException{
		...
		for (BIR bir : birs) {
			...
			birArray[0] = bir;
			if(biometricType.name().equalsIgnoreCase(BiometricType.IRIS.name()) ||
					biometricType.name().equalsIgnoreCase(BiometricType.FACE.name()) ||
					biometricType.name().equalsIgnoreCase(BiometricType.FINGER.name())) {
