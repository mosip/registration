...
public class CMDValidationProcessor {
	...
	public MessageDTO process(MessageDTO object, String stageName) {
		...
		try {
			...
			List<String> subProcessesList=Arrays.asList(subProcesses.split(","));
			if(subProcessesList.contains(registrationStatusDto.getRegistrationType())) {
				if (validateRegistrationCenterforSubProcess && validateCenter) {
					centerValidator.validate(primaryLanguagecode, regOsi, registrationStatusDto.getRegistrationId());
				}

				if (validateMachineforSubProcess && validateMachine) {
					machineValidator.validate(regOsi.getMachineId(), primaryLanguagecode, regOsi.getPacketCreationDate(),
							registrationStatusDto.getRegistrationId());
				}

				if (validateDeviceforSubProcess && validateDevice) {
