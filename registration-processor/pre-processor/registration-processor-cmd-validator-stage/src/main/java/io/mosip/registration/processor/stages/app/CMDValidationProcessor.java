...
import io.mosip.registration.processor.stages.cmdvalidator.utils.CMDUtils;
...
public class CMDValidationProcessor {
	...
	@Value("${mosip.registartion.processor.validateCenter}")
	private boolean validateCenter;

	@Value("${mosip.registartion.processor.validateMachine}")
	private boolean validateMachine;

	@Value("${mosip.registartion.processor.validateDevice}")
	...
	public MessageDTO process(MessageDTO object, String stageName) {
		...
		object.setMessageBusAddress(MessageBusAddress.CMD_BUS_IN);
		...
		try {
			...
			RegOsiDto regOsi = cmdUtils.getOSIDetailsFromMetaInfo(metaInfo);
			...
			if (validateCenter) {
				centerValidator.isValidCenterDetails(primaryLanguagecode, regOsi,
						registrationStatusDto.getRegistrationId());
			}

			if (validateMachine) {
				machineValidator.isValidMachine(regOsi.getMachineId(), primaryLanguagecode,
						regOsi.getPacketCreationDate(), registrationStatusDto.getRegistrationId());
			}

			if (validateDevice) {
				deviceValidator.isValidDevice(regOsi, registrationStatusDto.getRegistrationId());
			}
