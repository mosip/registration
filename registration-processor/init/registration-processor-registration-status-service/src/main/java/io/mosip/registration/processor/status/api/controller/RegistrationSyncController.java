		...
		public RegSyncResponseDTO buildRegistrationSyncResponse(List<SyncResponseDto> syncResponseDtoList)
			...
			for (SyncResponseDto syncResponseDto : syncResponseDtoList) {
				if (syncResponseDto.getStatus().equals(ResponseStatusCode.SUCCESS.toString())) {
					...
					} else if (syncResponseDto instanceof SyncResponseFailureV2Dto) {
						SyncErrDTO errors = new SyncErrDTO(((SyncResponseFailureV2Dto) syncResponseDto).getErrorCode(),
								((SyncResponseFailureV2Dto) syncResponseDto).getMessage());
						errors.setRegistrationId(((SyncResponseFailureV2Dto) syncResponseDto).getRegistrationId());
						errors.setStatus(syncResponseDto.getStatus());
